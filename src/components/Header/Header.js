    ...
    >
      <Toolbar className="header__nav">
        ...
        <IconButton
          ...
          onClick={() => setMenuOpen(!menuOpen)} // ✅ Now toggles menu open/close
        ...
        <Drawer
          anchor="top"
          open={menuOpen}
          onClose={() => setMenuOpen(false)} // ✅ Closes when clicking outside
            ...
            {['Home', 'Products', 'Services', 'Pricing', 'Contact'].map((text, index) => (
              <ListItem key={index} disablePadding>
                <ListItemButton
                  component={Link}
                  to={`/${text.toLowerCase()}`}
                  onClick={() => setMenuOpen(false)} // ✅ Closes when clicking a link
